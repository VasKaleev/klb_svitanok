   ...
   <!--подключение гугл аналитики для поисковой системы-->
	...
	<div id="wrapper"><!--начало wrapper-->
		...
		<!--бегущая строка-->
		    ...
		    <div  id="line_7" >
				<div>
				    <h2><span style="font-family: 'Vollkorn', Serif; color: crimson; font-size: 25px">Список спортсменов КЛБ Свитанок </h2></span><br>
					 ...
					 td:nth-child(2n) { color: SteelBlue;}
					 </style> 
                    ...
                    <?php
							...
							if (isset($_POST["FIO"])) {
							  ...
							  if (isset($_GET['red'])) {
								$sql = mysqli_query($link, "UPDATE `spisvit` SET `FIO` = '{$_POST['FIO']}',`datar` = '{$_POST['datar']}',`mt` = '{$_POST['mt']}',`dt` = '{$_POST['dt']}',`rabota` = '{$_POST['rabota']}',`br` = '{$_POST['br']}',`dolg` = '{$_POST['dolg']}',`tabp` = '{$_POST['tabp']}',`tel` = '{$_POST['tel']}',`gorod` = '{$_POST['gorod']}' WHERE `id_id`={$_GET['red']}");
							  } else {
							...
							//Удаляем, если что
							if (isset($_GET['del'])) {
							  $sql = mysqli_query($link, "DELETE FROM `spisvit` WHERE `id_id` = {$_GET['del']}");
							  if ($sql) {
						  ...
						  <form action="" method="post">
							<table>
							  <tr>
								<td>ФИО</td>
								<td><input type="text" size="35" name="FIO" value="<?= isset($_GET['red']) ? $product['FIO'] : ''; ?>"></td>
							  </tr>
							  <tr>
								<td>Дата рождения</td>
								<td><input type="text" size="10" name="datar" size="3" value="<?= isset($_GET['red']) ? $product['datar'] : ''; ?>"></td>
							  </tr>
							  <tr>
								<td>Мобильный тел</td>
								<td><input type="text" size="17" name="mt" size="3" value="<?= isset($_GET['red']) ? $product['mt'] : ''; ?>"></td>
							  </tr>
							  <tr>
								<td>Дом тел</td>
								<td><input type="text" size="7" name="dt" size="3" value="<?= isset($_GET['red']) ? $product['dt'] : ''; ?>"></td>
							  </tr>
							  <tr>
								<td>Работа</td>
								<td><input type="text" size="15" name="rabota" size="3" value="<?= isset($_GET['red']) ? $product['rabota'] : ''; ?>"></td>
							  </tr>
							  <tr>
								<td>Бриг</td>
								<td><input type="text" size="1" name="br" size="3" value="<?= isset($_GET['red']) ? $product['br'] : ''; ?>"></td>
							  </tr>
							  <tr>
								<td>Долж</td>
								<td><input type="text" size="25" name="dolg" size="3" value="<?= isset($_GET['red']) ? $product['dolg'] : ''; ?>"></td>
							  </tr>
							  <tr>
								<td>Таб перс</td>
								<td><input type="text" size="25" name="tabp" size="3" value="<?= isset($_GET['red']) ? $product['tabp'] : ''; ?>"></td>
							  </tr>
							   <tr>
								<td>Тел</td>
								<td><input type="text" size="25" name="tel" size="3" value="<?= isset($_GET['red']) ? $product['tel'] : ''; ?>"></td>
							  </tr>
							  <tr>
								<td>Город</td>
								<td><input type="text" size="25" name="gorod" size="3" value="<?= isset($_GET['red']) ? $product['gorod'] : ''; ?>"></td>
							  </tr>
							  <tr>
						  ...
						  <tr>
							...
							<td width="25%"><b id="r">ФИО</b></td>
							<td width="5%"><b id="r">Дата рождения</b></td>
							<td width="10%"><b id="r">Мобильный тел</b></td>
							<td width="3%"><b id="r">Дом тел</b></td>
							<td width="3%"><b id="r">Работа</b></td>
							<td width="1%"><b id="r">Бр</b></td>
							<td width="1%"><b id="r">Долж</b></td>
						  ...
						  <?php
						  //Получаем данные
						  $sql = mysqli_query($link, 'SELECT `id_id`, `FIO`, `datar`, `mt`, `dt`, `dt`, `rabota`, `br`, `dolg`, `tabp`, `tel`, `gorod`, `foto` FROM `spisvit` ORDER by `FIO`');
						  $np=0;
						 ...
						 //}
						  ...
						  <hr>
						  <p>Добавить фото по id_id спортсмена</p>
						  ...
						  <?php
							...
							$image = addslashes(file_get_contents($image));
													...
													mysqli_set_charset($db, "utf8");
													@mysqli_query ($db, 'set character_set_results = "utf8"');
													$result=mysqli_query($db,'SELECT * FROM `spisvit` '); // запрос на выборку
													while($row=mysqli_fetch_array($result)){
													$id_id=$row['id_id'];
													if ($id_id == $id_idf) 		{				
													$sql="UPDATE `spisvit` SET `foto` = '$image' WHERE `id_id`=$id_idf";
