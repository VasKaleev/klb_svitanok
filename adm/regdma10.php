...
<head>
  <meta charset="utf-8" />
	<meta name="description" content="КЛБ Свитанок-регистрация 10 км не первенство БМЗ">
	<meta name="keywords" content="КЛБ Свитанок-регистрация 10 км не первенство БМЗ">
	<link rel="stylesheet" href="../style.css" />
    <title>КЛБ Свитанок-регистрация 10 км не первенство БМЗ</title> 
   ...
   <!--подключение гугл аналитики для поисковой системы-->
	...
	<div id="wrapper"><!--начало wrapper-->
		...
		<!--бегущая строка-->
		    ...
		    <div  id="line_7" >
				<div>
				    <h2><span style="font-family: 'Vollkorn', Serif; color: crimson; font-size: 25px">Редактирование данных  участников пробега день металлурга г.Жлобин 10 км </h2></span><br>
                    ...
                    <?php
							...
							if (isset($_POST["fam"])) {	
							  ...
							  if (isset($_GET['red'])) {
								$sql = mysqli_query($link, "UPDATE `dme10km` SET `dist` = '{$_POST['dist']}',`fam` = '{$_POST['fam']}',`ima` = '{$_POST['ima']}',`otch` = '{$_POST['otch']}',`pol` = '{$_POST['pol']}',`rabota` = '{$_POST['rabota']}',`nomer` = '{$_POST['nomer']}',`datar` = '{$_POST['datar']}',`let` = '{$_POST['let']}',`paspnom` = '{$_POST['paspnom']}',`datvidp` = '{$_POST['datvidp']}',`kemvidp` = '{$_POST['kemvidp']}',`adres` = '{$_POST['adres']}',`klb` = '{$_POST['klb']}' WHERE `id_id`={$_GET['red']}");
							  } else {
								//Иначе вставляем данные, подставляя их в запрос
								$sql = mysqli_query($link, "INSERT INTO `dme10km` (`dist`,`fam`,`ima`,`otch`,`pol`,`rabota`, `nomer`,`datar`,`let`,`paspnom`,`datvidp`,`kemvidp`, `adres`, `klb`) VALUES ('{$_POST['dist']}','{$_POST['fam']}','{$_POST['ima']}','{$_POST['otch']}','{$_POST['pol']}','{$_POST['rabota']}', '{$_POST['nomer']}','{$_POST['datar']}','{$_POST['let']}','{$_POST['paspnom']}','{$_POST['datvidp']}','{$_POST['kemvidp']}', '{$_POST['adres']}', '{$_POST['klb']}')");
							  }
							...
							//Удаляем, если что
							if (isset($_GET['del'])) {
							  $sql = mysqli_query($link, "DELETE FROM `dme10km` WHERE `id_id` = {$_GET['del']}");
							  if ($sql) {
							...
							if (isset($_GET['red'])) {
							  $sql = mysqli_query($link, "SELECT `id_id`,`dist`, `fam`,`ima`,`otch`,`pol`,`rabota`, `nomer`,`datar`,`let`,`paspnom`,`datvidp`,`kemvidp`, `adres`, `klb` FROM `dme10km` WHERE `id_id`={$_GET['red']}");
						  ...
						  <form action="" method="post">
							<table>
							  <tr>
								<td>Дистанция
								 <input type="number" size="2" name="dist"  min="10" max="10"  step="0" value="<?= isset($_GET['red']) ? $product['dist'] : ''; ?>">
								</td>
							  </tr>
							  <tr>
								<td>Фамилия
						  ...
						  <?php
						  //Получаем данные
						  $sql = mysqli_query($link, 'SELECT `id_id`, `dist`,`fam`,`ima`,`otch`,`pol`,`rabota`, `nomer`,`datar`,`let`,`paspnom`,`datvidp`,`kemvidp`, `adres`, `klb` FROM `dme10km` ORDER by `let`,`pol`,`fam`,`nomer` ASC');
						  $np=0;
