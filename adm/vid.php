...
<head>
  <meta charset="utf-8" />
	<meta name="description" content="КЛБ Свитанок-режим админ-Видео">
	<meta name="keywords" content="КЛБ Свитанок-режим админ-Видео">
	<link rel="stylesheet" href="../style.css" />
    <title>КЛБ Свитанок-режим админ-Видео</title> 
   ...
   <!--подключение гугл аналитики для поисковой системы-->
	...
	<div id="wrapper"><!--начало wrapper-->
		...
		<!--бегущая строка-->
		    ...
		    <div  id="line_7" >
				<div>
				    <h2><span style="font-family: 'Vollkorn', Serif; color: crimson; font-size: 25px">Раздел Видео </h2></span><br>
                    ...
                    <?php
							...
							if (isset($_POST["name"])) {	
							  ...
							  if (isset($_GET['red'])) {
								$sql = mysqli_query($link, "UPDATE `video` SET `dataa` = '{$_POST['dataa']}',`href` = '{$_POST['href']}',`name` = '{$_POST['name']}' WHERE `id_id`={$_GET['red']}");
							  } else {
								//Иначе вставляем данные, подставляя их в запрос
								$sql = mysqli_query($link, "INSERT INTO `video` (`dataa`, `href`, `name`) VALUES ('{$_POST['dataa']}', '{$_POST['href']}', '{$_POST['name']}')");
							  }
							...
							//Удаляем, если что
							if (isset($_GET['del'])) {
							  $sql = mysqli_query($link, "DELETE FROM `video` WHERE `id_id` = {$_GET['del']}");
							  if ($sql) {
							...
							if (isset($_GET['red'])) {
							  $sql = mysqli_query($link, "SELECT `id_id`, `dataa`, `href`, `name` FROM `video` WHERE `id_id`={$_GET['red']}");
						  ...
						  <?php
						  //Получаем данные
						  $sql = mysqli_query($link, 'SELECT `id_id`, `dataa`, `href`, `name` FROM `video` ORDER by `dataa` DESC');
						  $np=0;
