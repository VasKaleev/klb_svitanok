...
<head>
  <meta charset="utf-8" />
	<meta name="description" content="КЛБ Свитанок-регистрация 5 км БМЗ">
	<meta name="keywords" content="КЛБ Свитанок-регистрация 5 км БМЗ">
	<link rel="stylesheet" href="../style.css" />
    <title>КЛБ Свитанок-регистрация 5 км БМЗ</title> 
   ...
   <!--подключение гугл аналитики для поисковой системы-->
	...
	<div id="wrapper"><!--начало wrapper-->
		...
		<!--бегущая строка-->
		    ...
		    <div  id="line_7" >
				<div>
				    <h2><span style="font-family: 'Vollkorn', Serif; color: crimson; font-size: 25px">Редактирование данных  участников пробега день металлурга г.Жлобин 5км Юноши и девушки – до 14 лет;14-16 лет;Мужчины –   70-74; 75 лет и старше.Женщины  –  65-69; 70 лет и старше. </h2></span><br>
                    ...
                    <?php
							...
							if (isset($_POST["fam"])) {	
							  ...
							  if (isset($_GET['red'])) {
								$sql = mysqli_query($link, "UPDATE `dm5km` SET `dist` = '{$_POST['dist']}',`fam` = '{$_POST['fam']}',`ima` = '{$_POST['ima']}',`otch` = '{$_POST['otch']}',`pol` = '{$_POST['pol']}',`rabota` = '{$_POST['rabota']}',`nomer` = '{$_POST['nomer']}',`datar` = '{$_POST['datar']}',`let` = '{$_POST['let']}',`paspnom` = '{$_POST['paspnom']}',`datvidp` = '{$_POST['datvidp']}',`kemvidp` = '{$_POST['kemvidp']}',`adres` = '{$_POST['adres']}',`klb` = '{$_POST['klb']}' WHERE `id_id`={$_GET['red']}");
							  } else {
								//Иначе вставляем данные, подставляя их в запрос
								$sql = mysqli_query($link, "INSERT INTO `dm5km` (`dist`,`fam`,`ima`,`otch`,`pol`,`rabota`, `nomer`,`datar`,`let`,`paspnom`,`datvidp`,`kemvidp`, `adres`, `klb`) VALUES ('{$_POST['dist']}','{$_POST['fam']}','{$_POST['ima']}','{$_POST['otch']}','{$_POST['pol']}','{$_POST['rabota']}', '{$_POST['nomer']}','{$_POST['datar']}','{$_POST['let']}','{$_POST['paspnom']}','{$_POST['datvidp']}','{$_POST['kemvidp']}', '{$_POST['adres']}', '{$_POST['klb']}')");
							  }
							...
							//Удаляем, если что
							if (isset($_GET['del'])) {
							  $sql = mysqli_query($link, "DELETE FROM `dm5km` WHERE `id_id` = {$_GET['del']}");
							  if ($sql) {
							...
							if (isset($_GET['red'])) {
							  $sql = mysqli_query($link, "SELECT `id_id`,`dist`, `fam`,`ima`,`otch`,`pol`,`rabota`, `nomer`,`datar`,`let`,`paspnom`,`datvidp`,`kemvidp`, `adres`, `klb` FROM `dm5km` WHERE `id_id`={$_GET['red']}");
						  ...
						  <form action="" method="post">
							<table>
							  ...
							  <td>Полных лет
								 <input type="number" size="5" name="let"  min="3" max="90" step="1" value="<?= isset($_GET['red']) ? $product['let'] : ''; ?>">
							   </td>
	    					  <td>Номер пасп
						  ...
						  <?php
						  //Получаем данные
						  $sql = mysqli_query($link, 'SELECT `id_id`, `dist`,`fam`,`ima`,`otch`,`pol`,`rabota`, `nomer`,`datar`,`let`,`paspnom`,`datvidp`,`kemvidp`, `adres`, `klb` FROM `dm5km` ORDER by `let`,`pol`,`fam`,`nomer` ASC');
						  $np=0;
