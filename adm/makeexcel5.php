...
$aSheet->setTitle('5 км день металлурга');
// РЁР°РїРєР° Рё С„СѓС‚РµСЂ (РїСЂРё РїРµС‡Р°С‚Рё)
$aSheet->getHeaderFooter()
       ->setOddHeader('5 км день металлурга работники БМЗ');
$aSheet->getHeaderFooter()
       ->setOddFooter('&L&B'.$aSheet->getTitle().'5 км день металлурга работники БМЗ');
...
$aSheet->setCellValue('L6','Место работы');
$aSheet->setCellValue('M6','Таб перс');
//$query = "SELECT `code`, `name`, `description`, `price` FROM `products` WHERE 1 LIMIT 10";
$res=mysqli_query($db,'SELECT * FROM `dme5kmbmz` ORDER by `let`,`pol`,`fam`,`nomer` ASC'); // Р·Р°РїСЂРѕСЃ РЅР° РІС‹Р±РѕСЂРєСѓ
...
while( $prd = mysqli_fetch_assoc($res) ) {
	...
	$aSheet->setCellValue('L'.($i+6), $prd['rabota']);
	$aSheet->setCellValue('M'.($i+6), $prd['tabp']);
...
$aSheet->getStyle('A1:M'.($i+5))->applyFromArray($style_wrap);
...
$aSheet->getStyle('A1:M1')->applyFromArray($style_header);
...
$aSheet->getStyle('A2:M2')->applyFromArray($style_slogan);
...
$aSheet->getStyle('A6:M6')->applyFromArray($style_hprice);
...
$aSheet->getStyle('A7:M'.($i+5))->applyFromArray($style_price);
/*
...
//$objWriter = iconv('Windows-1251', 'UTF-8', $objWriter);
$objWriter->save('den metallurga 5 km Bmz.xlsx');
//$objWriter->open('den metallurga 10 km.xlsx');
//$xls = new COM("Excel.Application"); // Создаем новый COM-объект
//$xls->Application->Visible = 1; // Делаем его видимым
//$xls->Open('adm\den metallurga 10 km.xlsx');
//echo mb_detect_encoding($pExcel);
//echo mb_detect_encoding($objWriter);
//echo mb_detect_encoding($pExcel);
echo 'Данные участников пробег день мет 5 км работники БМЗ выгружены успешно!';
echo '<a target="_blank" href="../adm/den metallurga 5 km Bmz.xlsx">Открыть файл</a><br/><br/>';
