...
<head>
  <meta charset="utf-8" />
	<meta name="description" content="КЛБ &laquo;Свитанок&raquo;-режим админ отзывы">
	<meta name="keywords" content="КЛБ &laquo;Свитанок&raquo;-режим админ отзывы">
	<link rel="stylesheet" href="../style.css" />
    <title>КЛБ &laquo;Свитанок&raquo;-режим админ отзывы</title> 
   ...
   <!--подключение гугл аналитики для поисковой системы-->
	...
	<div id="wrapper"><!--начало wrapper-->
		...
		<!--бегущая строка-->
		    ...
		    <div  id="line_7" >
				<div>
				    <h2><span style="font-family: 'Vollkorn', Serif; color: crimson; font-size: 25px">Раздел Отчёты о соревнованиях </h2></span><br>
                    ...
                    <?php
							...
							if (isset($_POST["name"])) {	
							  ...
							  if (isset($_GET['red'])) {
								$sql = mysqli_query($link, "UPDATE `otch` SET `dataa` = '{$_POST['dataa']}',`href` = '{$_POST['href']}',`name` = '{$_POST['name']}' WHERE `id_id`={$_GET['red']}");
							  } else {
								//Иначе вставляем данные, подставляя их в запрос
								$sql = mysqli_query($link, "INSERT INTO `otch` (`dataa`, `href`, `name`) VALUES ('{$_POST['dataa']}', '{$_POST['href']}', '{$_POST['name']}')");
							  }
							...
							//Удаляем, если что
							if (isset($_GET['del'])) {
							  $sql = mysqli_query($link, "DELETE FROM `otch` WHERE `id_id` = {$_GET['del']}");
							  if ($sql) {
							...
							if (isset($_GET['red'])) {
							  $sql = mysqli_query($link, "SELECT `id_id`, `dataa`, `href`, `name` FROM `otch` WHERE `id_id`={$_GET['red']}");
						  ...
						  <form action="" method="post">
							<table>
							  <tr>
								<td>Дата пробега</td>
								<td><input type="text" size="20" name="dataa" size="3" value="<?= isset($_GET['red']) ? $product['dataa'] : ''; ?>"></td>
							  </tr>
							  <tr>
						  ...
						  <?php
						  //Получаем данные
						  $sql = mysqli_query($link, 'SELECT `id_id`, `dataa`, `href`, `name` FROM `otch` ORDER by `dataa` DESC');
						  $np=0;
