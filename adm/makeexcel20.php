...
$aSheet->setTitle('20 км день металлурга');
// РЁР°РїРєР° Рё С„СѓС‚РµСЂ (РїСЂРё РїРµС‡Р°С‚Рё)
$aSheet->getHeaderFooter()
       ->setOddHeader('20 км день металлурга');
$aSheet->getHeaderFooter()
       ->setOddFooter('&L&B'.$aSheet->getTitle().'20 км день металлурга');
...
$aSheet->setCellValue('A2','дистанция 20 км');
...
$res=mysqli_query($db,'SELECT * FROM `dme20km` ORDER by `let`,`pol`,`fam`,`nomer` ASC'); // Р·Р°РїСЂРѕСЃ РЅР° РІС‹Р±РѕСЂРєСѓ
...
$objWriter->save('den metallurga 20 km.xlsx');
//$objWriter->open('den metallurga 10 km.xlsx');
//$xls = new COM("Excel.Application"); // Создаем новый COM-объект
//$xls->Application->Visible = 1; // Делаем его видимым
//$xls->Open('adm\den metallurga 20 km.xlsx');
//echo mb_detect_encoding($pExcel);
//echo mb_detect_encoding($objWriter);
//echo mb_detect_encoding($pExcel);
echo 'Данные участников пробег день мет 20 км выгружены успешно!';
echo '<a target="_blank" href="../adm/den metallurga 20 km.xlsx">Открыть файл</a><br/><br/>';
