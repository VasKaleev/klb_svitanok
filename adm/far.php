...
<head>
  <meta charset="utf-8" />
 	<meta name="description" content="КЛБ &laquo;Свитанок&raquo;-режим админ-файловый архив">
	<meta name="keywords" content="КЛБ &laquo;Свитанок&raquo;-админ сайта-файловый архив">
	<link rel="stylesheet" href="../style.css" />
    <title>КЛБ &laquo;Свитанок&raquo;-режим админ файловый архив</title> 
   ...
   <!--подключение гугл аналитики для поисковой системы-->
	...
	<div id="wrapper"><!--начало wrapper-->
		...
		<!--бегущая строка-->
		    ...
		    <div  id="line_7" >
				<div>
				    <h2><span style="font-family: 'Vollkorn', Serif; color: crimson; font-size: 25px">Файловый архив </h2></span><br>
				    <style>
					   table {
						<!--width: 100%;-->
                    ...
                    <?php
							...
							if (isset($_POST["name"])) {	
							  ...
							  if (isset($_GET['red'])) {
								$sql = mysqli_query($link, "UPDATE `farh` SET `datap` = '{$_POST['datap']}',`href` = '{$_POST['href']}',`name` = '{$_POST['name']}' WHERE `id_id`={$_GET['red']}");
							  } else {
								//Иначе вставляем данные, подставляя их в запрос
								$sql = mysqli_query($link, "INSERT INTO `farh` (`datap`, `href`, `name`) VALUES ('{$_POST['datap']}', '{$_POST['href']}', '{$_POST['name']}')");
							  }
							...
							//Удаляем, если что
							if (isset($_GET['del'])) {
							  $sql = mysqli_query($link, "DELETE FROM `farh` WHERE `id_id` = {$_GET['del']}");
							  if ($sql) {
							...
							if (isset($_GET['red'])) {
							  $sql = mysqli_query($link, "SELECT `id_id`, `datap`, `href`, `name` FROM `farh` WHERE `id_id`={$_GET['red']}");
						  ...
						  <?php
						  //Получаем данные
						  $sql = mysqli_query($link, 'SELECT `id_id`, `datap`, `href`, `name` FROM `farh` ORDER by `datap` DESC');
						  $np=0;
