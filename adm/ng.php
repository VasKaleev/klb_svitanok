...
<head>
  <meta charset="utf-8" />
	<meta name="description" content="КЛБ &laquo;Свитанок&raquo;-раздел админ награды">
	<meta name="keywords" content="КЛБ &laquo;Свитанок&raquo;-раздел админ награды">
	<link rel="stylesheet" href="../style.css" />
    <title>КЛБ &laquo;Свитанок&raquo;-раздел админ награды</title> 
   ...
   <!--подключение гугл аналитики для поисковой системы-->
	...
	<div id="wrapper"><!--начало wrapper-->
		...
		<!--бегущая строка-->
		    ...
		    <div  id="line_7" >
				<div>
				    <h2><span style="font-family: 'Vollkorn', Serif; color: crimson; font-size: 25px">Наши награды </h2></span><br>
				    <style>
					   table {
						<!--width: 100%;-->
                    ...
                    <?php
							...
							if (isset($_POST["name"])) {	
							  ...
							  if (isset($_GET['red'])) {
								$sql = mysqli_query($link, "UPDATE `nagr` SET `datap` = '{$_POST['datap']}',`href` = '{$_POST['href']}',`name` = '{$_POST['name']}' WHERE `id_id`={$_GET['red']}");
							  } else {
								//Иначе вставляем данные, подставляя их в запрос
								$sql = mysqli_query($link, "INSERT INTO `nagr` (`datap`, `href`, `name`) VALUES ('{$_POST['datap']}', '{$_POST['href']}', '{$_POST['name']}')");
							  }
							...
							//Удаляем, если что
							if (isset($_GET['del'])) {
							  $sql = mysqli_query($link, "DELETE FROM `nagr` WHERE `id_id` = {$_GET['del']}");
							  if ($sql) {
							...
							if (isset($_GET['red'])) {
							  $sql = mysqli_query($link, "SELECT `id_id`, `datap`, `href`, `name` FROM `nagr` WHERE `id_id`={$_GET['red']}");
						  ...
						  <form action="" method="post">
							<table>
							  <tr>
								<td>Дата пробега</td>
								<td><input type="text" size="20" name="dataa" size="3" value="<?= isset($_GET['red']) ? $product['datap'] : ''; ?>"></td>
							  </tr>
							  <tr>
						  ...
						  <tr>
							...
							<td width="160"><b id="r">Наши призеры</b></td>
						  ...
						  <?php
						  //Получаем данные
						  $sql = mysqli_query($link, 'SELECT `id_id`, `datap`, `href`, `name` FROM `nagr` ORDER by `datap` DESC');
						  $np=0;
