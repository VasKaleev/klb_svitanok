...
<head>
  <meta charset="utf-8" />
	<meta name="description" content="КЛБ Свитанок-регистрация участников 5 км работники БМЗ">
	<meta name="keywords" content="КЛБ Свитанок-регистрация участников 5 км работники БМЗ">
	<link rel="stylesheet" href="../style.css" />
    <title>КЛБ Свитанок-регистрация 5 км работники БМЗ</title> 
   ...
   <!--подключение гугл аналитики для поисковой системы-->
	...
	<div id="wrapper"><!--начало wrapper-->
		...
		<!--бегущая строка-->
		    ...
		    <div  id="line_7" >
				<div>
				    <h2><span style="font-family: 'Vollkorn', Serif; color: crimson; font-size: 25px">Редактирование данных  участников пробега день металлурга 5 км БМЗ г Жлобин </h2></span><br>
                    ...
                    <?php
							...
							if (isset($_POST["tabp"])) {	
							  ...
							  if (isset($_GET['red'])) {
								$sql = mysqli_query($link, "UPDATE `dme5kmbmz` SET `dist` = '{$_POST['dist']}',`fam` = '{$_POST['fam']}',`ima` = '{$_POST['ima']}',`otch` = '{$_POST['otch']}',`pol` = '{$_POST['pol']}',`rabota` = '{$_POST['rabota']}',`nomer` = '{$_POST['nomer']}',`datar` = '{$_POST['datar']}',`let` = '{$_POST['let']}',`tabp` = '{$_POST['tabp']}',`klb` = '{$_POST['klb']}' WHERE `id_id`={$_GET['red']}");
							  } else {
								//Иначе вставляем данные, подставляя их в запрос
								$sql = mysqli_query($link, "INSERT INTO `dme5kmbmz` (`dist`,`fam`,`ima`,`otch`,`pol`,`rabota`, `nomer`,`datar`,`let`, `tabp`, `klb`) VALUES ('{$_POST['dist']}','{$_POST['fam']}','{$_POST['ima']}','{$_POST['otch']}','{$_POST['pol']}','{$_POST['rabota']}', '{$_POST['nomer']}','{$_POST['datar']}','{$_POST['let']}', '{$_POST['tabp']}', '{$_POST['klb']}')");
							  }
							...
							//Удаляем, если что
							if (isset($_GET['del'])) {
							  $sql = mysqli_query($link, "DELETE FROM `dme5kmbmz` WHERE `id_id` = {$_GET['del']}");
							  if ($sql) {
							...
							if (isset($_GET['red'])) {
							  $sql = mysqli_query($link, "SELECT `id_id`,`dist`, `fam`,`ima`,`otch`,`pol`,`rabota`, `nomer`,`datar`,`let`, `tabp`, `klb` FROM `dme5kmbmz` WHERE `id_id`={$_GET['red']}");
						  ...
						  <form action="" method="post">
							<table>
							  <tr>
								<td>Дистанция
								 <input type="number" size="2" name="dist"  min="5" max="5"  step="0" value="<?= isset($_GET['red']) ? $product['dist'] : ''; ?>">
								</td>
							  </tr>
							  <tr>
								<td>Фамилия
	    					  ...
	    					  <td>Таб перс
								<input type="text" size="10" name="tabp"  value="<?= isset($_GET['red']) ? $product['tabp'] : ''; ?>">
							  </td>
							  <td>КЛБ
						  ...
						  <tr>
							...
							<td width="15"><b id="r">Таб перс</b></td>
							<td width="15"><b id="r">КЛБ</b></td>
						  </tr>
						  <?php
						  //Получаем данные
						  $sql = mysqli_query($link, 'SELECT `id_id`, `dist`,`fam`,`ima`,`otch`,`pol`,`rabota`, `nomer`,`datar`,`let`, `tabp`, `klb` FROM `dme5kmbmz` ORDER by `let`,`fam`,`nomer` ASC');
						  $np=0;
						  
						  while( $result = mysqli_fetch_array( $sql ) ){
							  $np++;
						 	   echo '<tr><td>&nbsp;'.$np.'</td><td>'.$result['dist'].'</td><td>'.$result['fam'].'</td><td>'.$result['ima'].'</td><td>'.$result['otch'].'</td><td>'.$result['pol'].'</td><td>'.$result['rabota'].'</td><td>'.$result['nomer'].'</td><td>'.$result['datar'].'</td><td>'.$result['let'].'</td><td>'.$result['tabp'].'</td><td>'.$result['klb'].'</td><td><a href=?del='.$result['id_id'].'>Удалить</a> - <a href=?red='.$result['id_id'].'>Редактировать</a></td></tr>';
    						  }
