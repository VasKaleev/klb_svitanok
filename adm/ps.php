...
<head>
  <meta charset="utf-8" />
	<meta name="description" content="КЛБ Свитанок-полезные статьи">
	<meta name="keywords" content="КЛБ Свитанок-полезные статьи">
	<link rel="stylesheet" href="../style.css" />
    <title>КЛБ Свитанок-полезные статьи</title> 
   ...
   <!--подключение гугл аналитики для поисковой системы-->
	...
	<div id="wrapper"><!--начало wrapper-->
		...
		<!--бегущая строка-->
		    ...
		    <div  id="line_7" >
				<div>
				    <h2><span style="font-family: 'Vollkorn', Serif; color: crimson; font-size: 25px">Полезные статьи </h2></span><br>
				    <style>
					   table {
						<!--width: 100%;-->
                    ...
                    <?php
							...
							if (isset($_POST["name"])) {	
							  ...
							  if (isset($_GET['red'])) {
								$sql = mysqli_query($link, "UPDATE `pst` SET `datap` = '{$_POST['datap']}',`href` = '{$_POST['href']}',`name` = '{$_POST['name']}' WHERE `id_id`={$_GET['red']}");
							  } else {
								//Иначе вставляем данные, подставляя их в запрос
								$sql = mysqli_query($link, "INSERT INTO `pst` (`datap`, `href`, `name`) VALUES ('{$_POST['datap']}', '{$_POST['href']}', '{$_POST['name']}')");
							  }
							...
							//Удаляем, если что
							if (isset($_GET['del'])) {
							  $sql = mysqli_query($link, "DELETE FROM `pst` WHERE `id_id` = {$_GET['del']}");
							  if ($sql) {
							...
							if (isset($_GET['red'])) {
							  $sql = mysqli_query($link, "SELECT `id_id`, `datap`, `href`, `name` FROM `pst` WHERE `id_id`={$_GET['red']}");
						  ...
						  <form action="" method="post">
							<table>
							  <tr>
								<td>Дата пробега</td>
								<td><input type="date" size="20" name="datap" size="3" value="<?= isset($_GET['red']) ? $product['datap'] : ''; ?>"></td>
							  </tr>
							  <tr>
								<td>Ссылка на пробег</td>
								<td><input type="url" size="64" name="href" size="3" value="<?= isset($_GET['red']) ? $product['href'] : ''; ?>"></td>
							  </tr>
							  <tr>
						  ...
						  <tr>
							...
							<td width="160"><b id="r">Название канала или статьи</b></td>
						  ...
						  <?php
						  //Получаем данные
						  $sql = mysqli_query($link, 'SELECT `id_id`, `datap`, `href`, `name` FROM `pst` ORDER by `datap` DESC');
						  $np=0;
						 ...
						 //}
						  while( $result = mysqli_fetch_array( $sql ) ){
							  $np++;
						 	   echo '<tr><td>&nbsp;'.$np.'</td><td>'.$result['datap'].'</td><td>'.$result['href'].'</td><td>'.$result['name'].'</td><td><a href=?del='.$result['id_id'].'>Удалить</a> - <a href=?red='.$result['id_id'].'>Редактировать</a></td></tr>';
