   ...
   <!--подключение гугл аналитики для поисковой системы-->
	...
	<div id="wrapper"><!--начало wrapper-->
		...
		<!--бегущая строка-->
			...
			<div id="conteiner">
				<div id="line_6">
					<ul id="navbar">
						...
						<li><a href="../obrsv.php">Опросы</a></li>
		    ...
		    <div  id="line_7" >
				<div>
				    <h2><span style="font-family: 'Vollkorn', Serif; color: crimson; font-size: 25px">Раздел Пресса о нас </h2></span><br>
                    ...
                    <?php
							...
							if (isset($_POST["name"])) {	
							  ...
							  if (isset($_GET['red'])) {
								$sql = mysqli_query($link, "UPDATE `pressa` SET `dataa` = '{$_POST['dataa']}',`href` = '{$_POST['href']}',`name` = '{$_POST['name']}' WHERE `id_id`={$_GET['red']}");
							  } else {
								//Иначе вставляем данные, подставляя их в запрос
								$sql = mysqli_query($link, "INSERT INTO `pressa` (`dataa`, `href`, `name`) VALUES ('{$_POST['dataa']}', '{$_POST['href']}', '{$_POST['name']}')");
							  }
							...
							//Удаляем, если что
							if (isset($_GET['del'])) {
							  $sql = mysqli_query($link, "DELETE FROM `pressa` WHERE `id_id` = {$_GET['del']}");
							  if ($sql) {
							...
							if (isset($_GET['red'])) {
							  $sql = mysqli_query($link, "SELECT `id_id`, `dataa`, `href`, `name` FROM `pressa` WHERE `id_id`={$_GET['red']}");
						  ...
						  <?php
						  //Получаем данные
						  $sql = mysqli_query($link, 'SELECT `id_id`, `dataa`, `href`, `name` FROM `pressa` ORDER by `dataa` DESC');
						  $np=0;
