...
<head>
  <meta charset="utf-8" />
    ...
    <title>КЛБ &laquo;Свитанок&raquo;-Полезные статьи</title> 
   ...
   <!--подключение гугл аналитики для поисковой системы-->
	...
	<div id="wrapper"><!--начало wrapper-->
		...
		<!--бегущая строка-->
		    ...
		    <div  id="line_7" >
					<h2><span style="color: crimson; font-size: 25px">Полезные статьи</span></h2>
					<?php
							...
							$result2 = mysqli_query($db,"SELECT * FROM `pst`;");
							...
							$result = mysqli_query($db,"SELECT * FROM `pst` Order by `datap` DESC
												  LIMIT $quantity OFFSET $list;");
							...
							for ($i = 0; $i<$num_result; $i++) {
								$row = mysqli_fetch_array($result);
									...
									echo '<h2><a target="_blank" href='.$link.'><span class="co">'.$data.'</span> '.$name.'</a></h2>';
