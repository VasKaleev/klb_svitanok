...
<head>
    ...
    <title>КЛБ &laquo;Свитанок&raquo;-Опросы </title>
...
<body id="fon">
    ...
    <div id="wrapper"><!--начало wrapper-->
        ...
        <div id="header">
            ...
            <div id="line_3">
                <img loading='lazy' class="heardb"
                    title="Если хочешь быть сильным — бегай, хочешь быть красивым — бегай, хочешь быть умным — бегай"
                    src="images/new2.png" alt="КЛБ Свитанок" width="120" height="120">
                <div id="email">
                ...
                <div id="pic">
                    <img loading='lazy' title="КЛБ Свитанок Жлобин" src="images/4.png" alt="КЛБ Свитанок Жлобин"
                        width="420" height="206">
                </div>
            </div>
            <div id="line_4">
                <h5>Наши партнеры:</h5>
        ...
        <div id="conteiner">
            <div id="line_6">
                <ul id="navbar">
                    ...
                    <li><a href="otzk.php">Отзывы и комментарии</a></li>
                    <li><a href="opros.php">Обратная связь</a></li>
                    <li><a href="klbm2019.php">КЛБ Матч СНГ</a></li>
                    <li><a href="ns.php">Новости спорта</a></li>
                </ul><br>
                <!-- <script defer>
                    ! function (e, t, r) {
                        e.PrcyCounterObject = r, e[r] = e[r] || function () {
                            (e[r].q = e[r].q || []).push(arguments)
                        };
                        var c = document.createElement("script");
                        c.type = "text/javascript", c.async = 1, c.src = t;
                        var n = document.getElementsByTagName("script")[0];
                        n.parentNode.insertBefore(c, n)
                    }(window, "//a.pr-cy.ru/assets/js/counter.sqi.min.js", "prcyCounter"), prcyCounter("svitanok.01sh.ru", "prcyru-sqi-counter", 0);
                </script> -->
                <!-- <div id="prcyru-sqi-counter"></div>
                <noscript><a href="https://pr-cy.ru/" target="_blank"><img
                            src="//a.pr-cy.ru/assets/img/analysis-counter.png" width="88" height="31"
                            alt="Анализ сайта"></a>
                </noscript> -->
            </div>
            ...
            <div id="line_7">
                ...
                if (!isset($_POST['fio']) and !isset($_POST['email'])) {
                    ...
                    <form class="comment" action="" method="post">
                        ...
                        <input class="form-control" type="text" name="email" placeholder="Укажите e-mail(Не публикуется)" required>
                        <input class="form-control" type="text" name="tel" placeholder="Укажите номер телефона(Не публикуется)" required>
                        <textarea class="form-control" rows="3" cols="10" name="comment"
                            placeholder="Напишите ваше сообщение сюда..."></textarea>
                        <div class="g-recaptcha" data-sitekey="6LfrNXkpAAAAADwf6eW-w9O4E_zw4DrqKvAuWZvP"></div>
                    ...
                    if (!empty($recaptcha)) {
                        ...
                        $secret = '6LfrNXkpAAAAAO7_XCx-3q6m2685ZtheoXi3VnJN';
                        ...
                        if ($curlData['success']) {
                            ...
                            $subject = 'Отзыв посетителя на странице отзывов сайта svitanok.01sh.ru '; // тема письма
                            // текст письма
                            $message = "ФИО: " . $fio . "<br/>";
                            $message .= "E-mail пользователя: " . $email . ' номер телефона ' . $tel . "<br/>";
                            $message .= "Текст отзыва(комментария): " . $comment;
                             // заголовок письма
                            ...
                            if (isset($_POST['fio']) and isset($_POST['email'])) {
                                ...
                                if (isset($_POST["fio"])) {
                                    ...
                                    $sql = mysqli_query($db, "INSERT INTO `otzk` (`fio`, `email`, `tel`, `comment`, `data_comment`) VALUES ('{$_POST['fio']}', '{$_POST['email']}', '{$_POST['tel']}', '{$_POST['comment']}', '{$data_comment}')");
                                }
                                //Если вставка прошла успешно
                                if ($sql) {
                                    echo '<div>Данные добавлены в таблицу отзывов!</div>';
                                } else {
                                    echo '<p>Произошла ошибка добавления данных в таблицу отзывов: ' . mysqli_error($sql) . '</p>';
                                }
                                ...
                                echo "<div>При отправке сообщения возникли ошибки</div>";
                            }
                        } else {
                            echo "<div>Подтвердите, что вы не робот и попробуйте еще раз</div>";
                        }
                    } else {
                        echo "<div>поставьте галочку в поле 'Я не робот' для отправки сообщения</div>";
                    }
                }
                ?>
                ...
                include('configinc.php');
                $sort = 'ORDER by `data_comment` DESC';
                $result = mysqli_query($db, 'SELECT `fio`,`email`,`comment`,`data_comment` FROM `otzk` ' . $sort . ''); // запрос на выборку
                //$result = mysqli_query($db, 'SELECT * FROM `otzk`'.$sort.'');
                $np = 0;
                while ($row = mysqli_fetch_array($result)) {
                    $fio = $row['fio'];
                    if ($fio) {
                        $np++;
                        echo '<div align="left" style="background: linear-gradient(to top, #CDEAA8 20%, #F4FFFF 50%, #CDEAA8 80%);"><font color=#03AA33 size="3pt">Написал:</font><font color=#2C4480 size="4pt"> &nbsp;' . $row["fio"] . '</font>&nbsp;<font color=#03AA33 size="4pt">(' . $row["data_comment"] . ') </font></div>';
                        echo '<div ><p id="ot" align="justify" style="background-color:#FDEAA8;">&nbsp;<font coor=#911E42 size="3pt">' . $row["comment"] . '</font></p></div>';
                        //echo '<div>'.$np.' ФИО: '.$fio.' Адрес эл почты: '.$row['email'].'<p>'.$row['comment'].'</p> </div>';
                    }
                }
                echo '<br><h4><font color=#03AA33 size="4pt">Всего отзывов: &nbsp;&nbsp;' . $np . '</font></h4>';
                mysqli_free_result($result);
                mysqli_close($db);
                ?>
                <!-- LikeBtn.com BEGIN -->
                <span class="likebtn-wrapper" data-theme="nero" data-lang="ru" data-white_label="true"
            ...
            <div id="line_8">
                <div class="col-12">
                    <h5 class="text-center  color1 mb-5">Наши друзья и партнеры</h5>
                </div>
                <div class="d-table">
                    <div class="d-tr">
                    ...
                    <div class="d-tr">
                        ...
                        <div class="d-td"><a target="_blank" href="http://www.probeg.org/"> <img
                                    title="ПроБЕГ в России и мире" src="images/5.gif" alt="ПроБЕГ в России и мире"></a>
                        </div>
                    </div>
                    <div class="d-tr">
                        ...
                        <div class="d-td"><a target="_blank" href="http://www.run4fun.by/"><img
                                    title="Клуб Run4Fun Барановичи" src="images/R4F.jpg" alt="Клуб Run4Fun Барановичи"
                                    width="45" height="45"></a></div>
                        <div class="d-td"><a target="_blank" href="https://metallurg.zhlobin.by"><img
                                    title="ХК Металлург Жлобин" src="images/mz.png" alt="ХК Металлург Жлобин" width="45"
                                    height="45"></a></div>
                    </div>
                    <div class="d-tr">
                ...
                <div><br><br>
                    <!--
                    <p><a href="https://clck.yandex.ru/redir/dtype=stred/pid=7/cid=1228/*https://yandex.by/pogoda/26966" target="_blank"><img src="https://info.weather.yandex.net/26966/4_white.ru.png?domain=ua" alt="Яндекс.Погода" /><img width="1" height="1" src="https://clck.yandex.ru/click/dtype=stred/pid=7/cid=1227/*https://img.yandex.ru/i/pix.gif" alt="" /></a> </p>
                  -->
                </div>
            </div>
        </div>
    ...
    <div id="line_9">
        <!--<p><font color=#FDEAA8 size="3pt" >© КЛБ &laquo;Свитанок&raquo; г.Жлобин, 2018-<?php echo date("Y"); ?>.</font></p>-->
        <h5>© КЛБ &laquo;Свитанок&raquo; г.Жлобин, 2018-
            <?php echo date("Y"); ?>.
        </h5>
        <div id="sch1">
        ...
        <div id="sch2">
            <p>
                ...
                <!--<a href="https://top.mail.ru/jump?from=3062632">
                    <img src="//top-fwz1.mail.ru/counter?id=3062632;t=479;l=1" style="border:0;" height="31" width="88"
        ...
        <div id="sch3">
            <p>
                ...
                <a href="https://metrika.yandex.ru/stat/?id=50681851&amp;from=informer" target="_blank"
                    rel="nofollow"><img
                        src="https://informer.yandex.ru/informer/50681851/3_1_FFA020FF_FF8000FF_0_pageviews"
                        style="width:88px; height:31px; border:0;" alt="Яндекс.Метрика"
                        title="Яндекс.Метрика: данные за сегодня (просмотры, визиты и уникальные посетители)"
                        class="ym-advanced-informer" data-cid="50681851" data-lang="ru" /></a>
                <!-- /Yandex.Metrika informer -->
                ...
                <noscript>
                    <div><img src="https://mc.yandex.ru/watch/50681851" style="position:absolute; left:-9999px;"
                            alt="" /></div>
                </noscript>
                <!-- /Yandex.Metrika counter -->
            </p>
            ...
            <p>
                ...
                <noscript>
                    <a href="http://click.hotlog.ru/?2573473" target="_blank">
                        <img src="http://hit20.hotlog.ru/cgi-bin/hotlog/count?s=2573473&im=313" title="HotLog"
                            alt="HotLog"></a>
                </noscript>
                <!-- /HotLog -->
            </p>
