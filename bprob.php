...
<head>
  <meta charset="utf-8" />
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1,user-scalable=yes">
  	<meta name="description" content="Ближайшие пробеги в республике Беларусь и за рубежом">
	<meta name="keywords" content="Ближайшие пробеги в республике Беларусь и за рубежом">
	<!--<link rel="stylesheet" href="style.css">-->
	<link rel="stylesheet" href="css/bootstrap.min.css">
	<link rel="stylesheet" href="style.css" media = "screen, print">
	<link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Lobster">
    <title>КЛБ &laquo;Свитанок&raquo;-Ближайшие пробеги</title> 
	...
	<div id="wrapper"><!--начало wrapper-->
		...
		<div id="header">
			...
			<a href="#down" class="pagedown">
        				<img src="img/up.svg" alt="down">
        	</a>
        	<!---->
			</div>
			<div id="line_3">  
		...
		<!--бегущая строка-->
			...
			<div id="conteiner">
				<div id="line_6">
					...
					<iframe src="https://calendar.google.com/calendar/embed?showTitle=0&amp;showNav=0&amp;showPrint=0&amp;showTabs=0&amp;showCalendars=0&amp;showTz=0&amp;height=150&amp;wkst=2&amp;bgcolor=%23ffcc99&amp;src=6ubcbrgkarss9h53bqtt2p1teg%40group.calendar.google.com&amp;color=%230F4B38&amp;src=p5a6remlvu4mlsv43jdi7lpiic%40group.calendar.google.com&amp;color=%23875509&amp;ctz=Europe%2FMinsk" style="border:solid 1px #777" width="200" height="150" frameborder="0" scrolling="no"></iframe>
        				<script>
		    ...
		    <div  id="line_7" >
		        <!--<a id="sp" href="#" onclick="window.print();return false;"><span style="color: crimson; font-size: 15px"><img title="Распечатать список пробегов" src="images/sp.jpg" alt="Распечатать список пробегов">  Распечатать список пробегов</span></a><br>-->
				<?php
                ...
                $today = date("Y-m-d");
				//$today = DATE(NOW());
				//echo 'проверка'.date("Y-m-d");
				//echo 'проверка'.date("Y-m-d");
				//WHERE `id_id`={$_GET['red']}
				//SELECT * FROM `novosti` WHERE `datap` >curdate() ORDER BY `datap` DESC 
                //$result2 = mysqli_query($db,'SELECT * FROM `novosti` WHERE `datap` >= curdate()');
                $result=mysqli_query($db,"SET @@lc_time_names = 'ru_RU'");
                $today = date("Y-m-d H:i:s");
                //$result2 = mysqli_query($db,'SELECT `dataa`, date(`dataa`) as data1, `name`, `note`, `href`, `img` FROM `novosti` WHERE `dataa` >= curdate()');
                $result2 = mysqli_query($db,'SELECT `dataa`, date(`dataa`) as data1, `name`, `note`, `href`, `img` FROM `novosti` WHERE `dataa` >= $today');
                $num = mysqli_num_rows($result2);
				//echo 'количество строк'.$num;
                ...
                $result = mysqli_query($db,"SELECT `dataa`, LOWER (DATE_FORMAT(`dataa`,'%e %M %Y')) as data1, `name`, `note`, `href`, `img` FROM `novosti` WHERE `dataa` >= curdate() Order by `dataa` ASC
                                      LIMIT $quantity OFFSET $list;");
                //$result = mysqli_query($db,"SELECT `dataa`, LOWER (DATE_FORMAT(`dataa`,'%e %M %Y')) as data1, `name`, `note`, `href`, `img` FROM `novosti` Order by `dataa` ASC
                //                      LIMIT $quantity OFFSET $list;");
                ...
                for ($i = 0; $i<$num_result; $i++) {
                    $row = mysqli_fetch_array($result);
							...
							//echo 'вывод строк '.$i;
							echo '
							<h5>Адрес пробега на карте</h5>
                            <a style="margin:0px 20px" target="_blank" href="karta.php/"><img class="heardb" title="Адрес ближайшего пробега" src="images/mp.png" alt="Адрес пробега" width="90" height="90"/></a><br>';
							}	
							...
							$link=$row["href"];
                			$img=$row["img"];  
                   		//echo '<h2><a target="_blank" href='.$link.'>'.$data.' '.$name.'</a></h2>';
                   		setlocale(LC_ALL, 'ru_RU', 'ru_RU.UTF-8', 'ru', 'russian'); 
                   		echo '<h2><a target="_blank" href='.$link.'><span class="co">'.$data1.'</span> '.$name.'</a></h2>';
						if ($dataa!="0000-00-00 00:00:00") {
							
                            	$check_time = strtotime($dataa) - time();
								$days = floor($check_time/86400);
								$hours = floor(($check_time%86400)/3600);
								$minutes = floor(($check_time%3600)/60);
								$seconds = $check_time%60; 
								if($check_time >= 0){
								echo '<font color=#1A7889 size="4pt">До пробега осталось: '.$days.' дн '.$hours.' ч '.$minutes.' мин '.$seconds.' сек</font>';
								}
                   		        if($check_time <= 0){
								echo '<font color=#1A7889 size="4pt">Пробег уже начался: '.abs($hours). ' часов '.abs($minutes).' минут  назад</font>';
								}
								}
                   		        echo  '<p><img class="poz" title="'.$name.'" src="data:image/jpeg;base64,'.base64_encode( $img ).'"  alt="'.$name.'" width="40" height="47"><b> '.$row["note"].'</b></p>';
    		    		}
    		    		
                        //echo '<br/><b><h4 id="page"><font color=crimson size="4pt">Страницы:&nbsp;&nbsp;</b></font> ';
                        echo '<br/><b><h4 id="page">Страницы:&nbsp;&nbsp;</b> ';
                ...
                for ($j = 1; $j<$pages; $j++) {
                    ...
                    if ($j>=$start && $j<=$end) {
                        ...
                        if ($j==($page+1))
                        echo '<a href="' . $_SERVER['SCRIPT_NAME'] . '?page=' . $j . '"><strong style="color: #000; background-color: #fdeaa8; padding: 6px 12px; border: 1px #aaa solid; border-radius: 50%;">' . $j . '</strong></a> &nbsp; ';
                ...
                //echo 'проверка'.$_SERVER['SCRIPT_NAME'].'путь';
    ...
    <div  id="line_8" >
                <!-- clock widget start -->
                <!--<script> var css_file=document.createElement("link"); css_file.setAttribute("rel","stylesheet"); css_file.setAttribute("type","text/css"); css_file.setAttribute("href","//s.bookcdn.com//css/cl/bw-cl-180x170r1.css"); document.getElementsByTagName("head")[0].appendChild(css_file); </script> <div id="tw_11_629461312"><div style="width:120px; height:140px; margin: 0 auto;"><a href="https://nochi.com/time/zhlobin-w190832">Жлобин</a><br/></div></div> <script type="text/javascript"> function setWidgetData_629461312(data){ if(typeof(data) != 'undefined' && data.results.length > 0) { for(var i = 0; i < data.results.length; ++i) { var objMainBlock = ''; var params = data.results[i]; objMainBlock = document.getElementById('tw_'+params.widget_type+'_'+params.widget_id); if(objMainBlock !== null) objMainBlock.innerHTML = params.html_code; } } } var clock_timer_629461312 = -1; </script> <script type="text/javascript" charset="UTF-8" src="https://widgets.booked.net/time/info?ver=2&domid=589&type=11&id=629461312&scode=124&city_id=w190832&wlangid=20&mode=2&details=0&background=ffffff&color=570b57&add_background=ffffff&add_color=333333&head_color=ffffff&border=0&transparent=0"></script>-->
                <!-- clock widget end -->
         <h5>Наши друзья и партнеры: </h5>
          <div class="d-table">
           <div class="d-tr">
             ...
             <div class="d-td"><a target="_blank" rel="noopener noreferrer" href="https://crazy-dev.pro/"><img title="Студия Creazy dev" src="images/cr.png"  alt="Студия Creazy dev"></a></div>
             <div class="d-td"><a rel="noopener noreferrer" href="https://runin.by/"><img title="Беговая лига - Runln" src="images/runln.png"  alt="Беговая лига - Runln"></a></div>
           </div>
           <div class="d-tr">
		   ...
		   <div id="line_9" >
		       <!--<h4>© КЛБ &laquo;Свитанок&raquo; г.Жлобин, 2018-2019.</h4>-->
		       <h4>© КЛБ &laquo;Свитанок&raquo; г.Жлобин, 2018-<?php echo date("Y");?>.</h4>
		       <div id="sch1">  
